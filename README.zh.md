...
| `prototype-smith` | sonnet | 头脑风暴与原型、实现计划 | 被限定在新建的可丢弃文件内;一次即可产出 N 个真正发散的方向 |
| `ledger-keeper` | opus | Cartographer 模式 | 未知台账的独立记账员;对遗憾打分,把各象限的清晰度打分汇总成加权歧义值,并在双重（覆盖度+歧义度）门控上作出裁决,而不评判自己的产出——裁决交给最强的模型 |
| `quiz-master` | sonnet | 测验、推介与讲解 | 未参与该改动撰写的"新眼睛"考官;专门追问作者自己会略过的地方 |
