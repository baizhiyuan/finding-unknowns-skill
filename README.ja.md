...
| `prototype-smith` | sonnet | ブレインストーミング&プロトタイピング、実装計画 | 新規の使い捨てファイルにサンドボックス化されている。1回のパスで真に発散したN個の方向性を生み出す |
| `ledger-keeper` | opus | Cartographer モード | 未知の台帳を独立して管理する記帳係。後悔(regret)をスコアリングし、象限ごとの明瞭度を重み付き曖昧さスコアへと変換し、自らの作業を採点することなくカバレッジ+曖昧さの二重ゲートを判定する — この判定には最も強力なモデルが充てられる |
| `quiz-master` | sonnet | クイズ、ピッチ&解説 | 変更を作成していない、先入観のない試験官。作成者が見落としがちな点を突く |
...
数、Cartographer のラウンド上限)は、[`SKILL.md`](skills/finding-unknowns/SKILL.md) 内の
**Defaults** テーブルに記載されており、平易な言葉でスキルに伝えることで上書きできます。
